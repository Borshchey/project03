...
Для передачи данных используйте классы `ru.itmo.lessons.lesson25.common.addition.Message`
...
3. Клиентская программа завершает работу по желанию пользователя (ввод exit в консоль) или, если сервер перестал
   отвечать.
4. Клиент в отдельном потоке получает данные от пользователя, создает экземпляр addition.Message и отправляет сообщение на
   сервер.
